...
def main():
    if len(sys.argv) < 2:
        print("Uso: python3 otp.py <setup|enc|dec> ...")
        return
    ...
    if op == "setup":
        if len(sys.argv) != 4:
            print("Uso: python3 otp.py setup <n_bytes> <ficheiro_chave>")
            return
        setup(int(sys.argv[2]), sys.argv[3])

    elif op == "enc":
        if len(sys.argv) != 4:
            print("Uso: python3 otp.py enc <ficheiro_ptxt> <ficheiro_chave>")
            return
        cifrar(sys.argv[2], sys.argv[3])

    elif op == "dec":
        if len(sys.argv) != 4:
            print("Uso: python3 otp.py dec <ficheiro_ctxt> <ficheiro_chave>")
            return
